...
plt.grid();
plt.show();
full_im_array_1 = full_im_array_1[375:525, 275:525, :]
...
full_im_array_2 = full_im_array_2[200:200 + size, :size, :]  # make square
full_im_array_2 = full_im_array_2[200:400, 300:500, :]
...
plt.imshow(t2im((rescale(h_image)) >= 0.5), cmap='gray', alpha=0.5)
plt.title("h")
ax = plt.gca()
ax.set_axis_off()
ax.margins(0, 0)
...
plt.imshow(t2im((q_image) >= 2.5), cmap='gray', alpha=0.5)
plt.xticks([])
plt.yticks([])
